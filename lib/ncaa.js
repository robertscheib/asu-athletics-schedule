...
// Year is derived, not hardcoded (a literal 2026 would go stale next season):
// try the current calendar year's bracket page, and if it doesn't exist yet
// (early in the year, before the field is announced) fall back to last
// season's. Only baseball is wired end-to-end today; sport is a parameter so
// another bracket can be added without touching the scrape logic.
async function getNcaaConfig(sport = 'baseball') {
  return configCache.getOrFetch(`config:${sport}`, 6 * 60 * 60 * 1000, async () => {
    const year = new Date().getFullYear();
    try {
      return await _fetchBracketConfig(sport, year);
    } catch (err) {
      console.warn(`[ncaa] ${sport}/${year} bracket config failed (${err.message}) — trying ${year - 1}`);
      return _fetchBracketConfig(sport, year - 1);
    }
  });
}

async function _fetchBracketConfig(sport, year) {
  const r = await fetch(`https://www.ncaa.com/brackets/${sport}/d1/${year}`, {
  ...
  console.log(`[ncaa] Config loaded: ${sport}/${year}, championshipId=${championshipId}, gqlHost=${gqlHost}, sections=${regionalSections.length}`);
  return config;
}
