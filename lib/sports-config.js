...
const ESPN_SPORT_SLUGS = {
  ...
  // College soccer lives under league codes (verified 2026-07: the
  // college-soccer-women/-men slugs return HTTP 400 for both scoreboard and
  // summary). Bare 'Soccer' is the feed's sport tag — ASU fields women's only.
  "Women's Soccer":       'soccer/usa.ncaa.w.1',
  "Men's Soccer":         'soccer/usa.ncaa.m.1',
  'Soccer':               'soccer/usa.ncaa.w.1',
...
const SPORT_CONFIG = [
  ...
  // NOTE: 'baseball/college-softball' looks wrong but is CORRECT for the
  // team-schedule/scoreboard endpoints (verified 2026-07: returns ASU Sun
  // Devils softball; 'softball/college-softball' 400s for the schedule).
  // The summary endpoint uses the softball/ prefix — see ESPN_SPORT_SLUGS.
...
// dbSport must equal the feed's sport tag or live games never match DB events:
// the feed tags soccer as bare 'Soccer', and ASU has no men's soccer program,
// so there is no men's entry — its scoreboard can never contain an ASU game.
// (Old 'soccer/womens-college-soccer'/'mens-college-soccer' paths 400, 2026-07.)
const LIVE_EXTRA_SPORTS = [
  ...
  { dbSport: 'Soccer',             espnPath: 'soccer/usa.ncaa.w.1',                  fallSport: true  },
];
