...
// buildTournaments runs inside the /api/live path — cache its per-game ESPN
// summary fetches (and pool standings) so tournament days don't add uncached
// sequential HTTP round-trips to every live poll.
const SUMMARY_TTL = 60_000;
const _summaryCache = new TtlCache();       // espnEventId → summary JSON
const _poolStandingsCache = new TtlCache(); // espnPath    → standings[] | null

async function _fetchSummary(espnPath, espnEventId) {
  const cached = _summaryCache.get(espnEventId);
  if (cached !== undefined) return cached;
  const url = `${ESPN_BASE}/${espnPath}/summary?event=${espnEventId}`;
  ...
  _summaryCache.set(espnEventId, data, SUMMARY_TTL);
  return data;
}
...
function inferRounds(games) {
  // Ids must be stable across 30s polls (random ids force re-renders).
  const matchups = games.map((g, i) => ({
    id: g.espnEventId || g.dbEventId || `m-${i}`,
...
function buildBracketRoundsFromSummaries(group, summaries) {
  ...
  for (const g of group.games) {
    ...
    roundMap[roundName].matchups.push({
      id: g.espnEventId || g.dbEventId || `m-${roundName}-${roundMap[roundName].matchups.length}`,
...
async function fetchPoolStandings(espnPath) {
  const cached = _poolStandingsCache.get(espnPath);
  if (cached !== undefined) return cached;
  const result = await _fetchPoolStandingsUncached(espnPath);
  _poolStandingsCache.set(espnPath, result, SUMMARY_TTL);
  return result;
}

async function _fetchPoolStandingsUncached(espnPath) {
...
async function buildTournaments(games) {
  ...
  for (const group of Object.values(groups)) {
    ...
    // Fetch ESPN summaries for each tournament game to get round/series
    // context — in parallel, through the 60s cache above.
    const summaries = [];
    await Promise.all(group.games.map(async g => {
      if (!g.espnEventId) return;
      try {
        const data = await _fetchSummary(cfg.espnPath, g.espnEventId);
        summaries.push({ gameId: g.espnEventId, data });
      } catch (err) {
...
async function detectActiveTournaments() {
  ...
  for (const group of Object.values(groups)) {
    ...
    const games = group.events.map(event => ({
      ...
      sport: event.sport,
      title: event.title,
      // No live scoreboard here (DB fallback path): a started game without a
      // result is presumed live for 5h, then final — plain start_date < now
      // showed in-progress games as FINAL.
      state: event.result ? 'final'
        : event.start_date > nowTs ? 'upcoming'
        : nowTs - event.start_date < 5 * 3600 ? 'live'
        : 'final',
